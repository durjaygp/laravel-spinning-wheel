@extends('backEnd.master')
@section('title','Spin Winner List')
@section('content')
    <div class="container-fluid">
        <div class="card bg-light-info shadow-none position-relative overflow-hidden">
            <div class="card-body px-4 py-3">
                <div class="row align-items-center">
                    <div class="col-9">
                        <h4 class="fw-semibold mb-8">@yield('title')</h4>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a class="text-muted " href="{{route('admin.index')}}">Dashboard</a></li>
                                <li class="breadcrumb-item" aria-current="page">Spin Winner List</li>
                            </ol>
                        </nav>
                    </div>
                    <div class="col-3">
                        <div class="text-center mb-n5">
                            <img src="{{asset('back')}}/assets/images/breadcrumb/ChatBc.png" alt="" class="img-fluid mb-n4">
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="widget-content searchable-container list">
            <!-- --------------------- start Contact ---------------- -->
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-4 ">
                            <h2>Spin Winner List</h2>
                        </div>
                    </div>
                </div>
                @if(session('success'))
                    <div class="alert alert-success">
                        {{ session('success') }}
                    </div>
                @endif

                @if(session('error'))
                    <div class="alert alert-danger">
                        {{ session('error') }}
                    </div>
                @endif
                <div class="card-body">
                    <div class="row">
                        <div class="col-12">
                            <div class="table-responsive">
                                <table id="zero_config"
                                       class="table border table-striped table-bordered text-nowrap table-responsive">
                                    <thead>
                                    <!-- start row -->
                                    <tr>
                                        <th>#</th>
                                        <th>User Details</th>
                                        <th>Skin Name</th>
                                        <th>Skin Image</th>
                                        <th>Status</th>
                                    </tr>
                                    <!-- end row -->
                                    </thead>
                                    <tbody>
                                    @foreach($users as $row)
                                        <!-- start row -->
                                        <tr>
                                            <td>{{$loop->iteration}}</td>
                                            <td>
                                                <p>Name: {{ $row->user->name }}</p>
                                                <p>Email: {{ $row->user->email }}</p>
                                                <p>Username: {{ $row->user->username }}</p>
                                            </td>
                                            <td> {{$row->game->title}}</td>

                                            <td><img src="{{asset($row->game->image)}}" alt=""></td>
                                            <td>
                                                @if($row->status == 1)
                                                    <span class="badge bg-secondary">Active</span>
                                                @elseif($row->status == 2)
                                                    <span class="badge bg-danger">Inactive</span>
                                                @endif
                                            </td>
                                            {{--                                            <td>--}}
                                            {{--                                                <div class="action-btn">--}}
                                            {{--                                                    <a href="#"--}}
                                            {{--                                                       onclick="event.preventDefault();--}}
                                            {{--                                                           if (confirm('Are you sure you want to delete?'))--}}
                                            {{--                                                           document.getElementById('delete-form-{{ $row->id }}').submit();"--}}
                                            {{--                                                       class="btn btn-sm btn-danger text-white delete ms-2">--}}
                                            {{--                                                        <i class="ti ti-trash fs-5"></i>--}}
                                            {{--                                                    </a>--}}

                                            {{--                                                    <form id="delete-form-{{ $row->id }}" action="{{ route('user-delete', $row->id) }}" method="get" style="display: none;">--}}
                                            {{--                                                        @csrf--}}
                                            {{--                                                    </form>--}}

                                            {{--                                                </div>--}}
                                            {{--                                            </td>--}}
                                        </tr>
                                        <!-- end row -->
                                    @endforeach
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>
@endsection
